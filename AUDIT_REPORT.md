...
6. **Medium:** Cache Corruption. A direct file write to `.build_hash` caused risks of leaving an invalid hash string during interruption or crashes. *Fix implemented: Atomic writes now use a temporary `.tmp` file structure before overwriting via `replace()`.*
7. **Medium:** Incremental Build Inaccuracy. The build check incorrectly only evaluated if `.build_hash` matched. Thus, manually deleted build outputs skipped compilation while remaining absent. *Fix implemented: The cache logic now correctly ensures the final `BINARY_NAME` artifact exists before skipping the build.*
